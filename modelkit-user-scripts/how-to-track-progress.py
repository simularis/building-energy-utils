...
def get_stats_by_folder(root: Path,
    ...
    if list_unfinished:
        ...
        stats.to_csv("sim_tracking_stats.csv")
...
def is_interrupted(sim_folder: Path):
    """ Return true if the simulation in the given folder looks like it was interrupted."""
    

def delete_interrupted(root:Path, prompt=True):
    """ Utility function for command line.
    WARNING: This function deletes interrupted simulation files.

    How: If modelkit is interrupted (ctrl-C), then it may leave temporary folders. E.g.,
    NG-SpaceHtg_eq-Boiler-Cond-TE-93\instance0329055941\log.txt
    "Run interrupted!"

    In this case, delete the temporary folder and output files.
    Don't delete the composed model.

    """
    sim_folders = [p.parent for p in root.glob("**/runs/**/instance.pxv")]


    unfinished_list = [d.ha]
    for stem in unfinished_list:
        print(stem)
        for name in names:
            f = Path(stem).joinpath(name)
